...
class SdToggle extends SdElement {
  ...
  styles() {
    return `
      ...
      button { display: inline-flex; align-items: center; gap: 12px; padding: 0; border: 0; background: transparent; cursor: pointer; }
      button:focus-visible { outline: 3px solid var(--sd-sun); outline-offset: 4px; border-radius: var(--sd-r-sm); }
  ...
  template() {
    ...
    return `<button type="button" role="switch" aria-checked="${this.battr("checked")}">
  ...
  afterRender() {
    this.shadowRoot.querySelector("button")?.addEventListener("click", () => {
      ...
      this.dispatchEvent(new CustomEvent("change", { bubbles: true, composed: true, detail: { checked: this.battr("checked") } }));
    });
  }
}
