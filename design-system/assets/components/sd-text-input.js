...
class SdTextInput extends SdElement {
  ...
  styles() {
    return `
      :host { display: block; }
      label { display: block; }
      label span { display: block; font-size: var(--sd-fs-sm); font-weight: var(--sd-fw-medium); color: var(--sd-ink-soft); margin-bottom: 6px; padding-left: 4px; }
      input {
        width: 100%;
        padding: 12px 14px;
  ...
  template() {
    ...
    return `
      ...
      <input type="${type}" value="${value}" placeholder="${ph}" /></label>
