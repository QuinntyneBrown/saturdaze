...
public sealed class RateWeekendCommandHandler : IRequestHandler<RateWeekendCommand, WeekendDto>
{
    private readonly IAppDbContext _db;
    private readonly ICurrentFamilyAccessor _current;
    private readonly WeekendForecastService _forecast;

    public RateWeekendCommandHandler(IAppDbContext db, ICurrentFamilyAccessor current, WeekendForecastService forecast)
    {
        _db = db;
        _current = current;
        _forecast = forecast;
    ...
    public async Task<WeekendDto> Handle(RateWeekendCommand request, CancellationToken cancellationToken)
    {
