/* Saturdaze mocks v2 — the only script.
   1. Injects the icon sprite (40 symbols, ids "i-<name>") so pages can use
      <svg class="icon"><use href="#i-lock"/></svg> from file:// or http.
   2. ADR-005: tracks the iOS Safari bottom chrome height into
...
(function () {
  ...
  /* ---------- 1. Icon sprite (paths copied from docs/mocks/components/sd-icon.js;
                 edit + trash from frontend/projects/components/src/lib/icon/icon.ts) */
  var ICONS = {
  ...
  function injectSprite() {
    ...
    host.innerHTML =
      '<svg id="sd-sprite" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" ' +
  ...
  /* ---------- 2. ADR-005 — bottom chrome height into --sd-chrome-bottom */
  function trackBottomChrome() {
    ...
    var update = function () {
      ...
      var bottomChrome = Math.max(0, layoutHeight - visibleBottom);
  ...
  /* ---------- 3. Scrolled top bar */
  function trackScrolledTopbar() {
    var bars = document.querySelectorAll(".topbar, .sitebar");
    if (!bars.length) return;
    var update = function () {
      var scrolled = window.scrollY > 4;
      for (var i = 0; i < bars.length; i++) {
        bars[i].toggleAttribute("data-scrolled", scrolled);
      }
    };
  ...
  /* ---------- 4. legal.html — Terms / Privacy switch on the hash */
  function trackLegalHash() {
    var docs = document.querySelectorAll("[data-doc]");
    if (!docs.length) return;
    var update = function () {
      var want = location.hash === "#privacy" ? "privacy" : "terms";
      document.body.setAttribute("data-doc", want);
      var chips = document.querySelectorAll('.doc-switch a[href^="#"]');
      for (var i = 0; i < chips.length; i++) {
        var isCurrent = chips[i].getAttribute("href") === "#" + want;
        if (isCurrent) chips[i].setAttribute("aria-current", "page");
