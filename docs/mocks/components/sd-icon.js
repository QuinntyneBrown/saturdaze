// <sd-icon name="..."> — single source of truth for iconography.
// Skeleton uses inline SVG paths from a small built-in set so the mock has
// no network deps. In implementation this can swap to an SVG sprite.
...
class SdIcon extends SdElement {
  ...
  styles() {
    return `
      ...
      svg { width: var(--_size); height: var(--_size); display: block; }
      svg * { fill: none; stroke: currentColor; stroke-width: 1.7; stroke-linecap: round; stroke-linejoin: round; }
      :host([filled]) svg path, :host([filled]) svg circle, :host([filled]) svg rect { fill: currentColor; stroke: none; }
    `;
  }

  template() {
    const name = this.attr("name", "sparkle");
    const size = this.attr("size", "20");
    const path = ICONS[name] || ICONS.sparkle;
    return `
      <style>:host { --_size: ${size}px; }</style>
      <svg viewBox="0 0 24 24" aria-hidden="true">${path}</svg>
    `;
  }
}

customElements.define("sd-icon", SdIcon);
